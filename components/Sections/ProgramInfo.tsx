import {
  BookOpen,
  Lightbulb,
  Users,
  Award,
  Target,
  Compass,
} from "lucide-react";
import type { VisionMission } from "@/types/visionMission";

interface ProgramInfoProps {
  visionMission?: VisionMission[];
  prodi?: "S1" | "D3";
  showVisionMission?: boolean;
}

// Fallback statis jika API belum ada data / error
const STATIC_VM: VisionMission = {
  id: 0,
  vision: "Menjadi program studi Teknik Informatika terdepan dalam menghasilkan profesional yang inovatif, berkarakter, dan berdaya saing global",
  mission: "Pendidikan berkualitas tinggi di bidang TI.\nPenelitian relevan dengan kebutuhan industri.\nPengabdian masyarakat berbasis transfer teknologi.",
  createdAt: "",
  updatedAt: ""
};

const features = [
  {
    icon: BookOpen,
    title: "Kurikulum Modern",
    description:
      "Materi pembelajaran adaptif sesuai standar industri global terkini.",
  },
  {
    icon: Lightbulb,
    title: "Riset & Inovasi",
    description:
      "Pusat pengembangan solusi teknologi yang berdampak bagi masyarakat.",
  },
  {
    icon: Users,
    title: "Koneksi Industri",
    description:
      "Jejaring luas dengan perusahaan teknologi untuk pengalaman praktis.",
  },
  {
    icon: Award,
    title: "Rekam Jejak",
    description:
      "Alumni sukses berkarier di startup unicorn dan perusahaan multinasional.",
  },
];

export default function ProgramInfo({
  visionMission = [],
  prodi,
  showVisionMission = true,
}: ProgramInfoProps) {
  const activeData = visionMission.length > 0 ? visionMission[0] : STATIC_VM;
  const missionPoints = activeData.mission.split('\n').filter(item => item.trim() !== "");

  const getCopywriting = () => {
    if (prodi === "S1") {
      return {
        tag: "Program Sarjana (S1) Teknik Informatika",
        headline: "Fokus Akademik & Keilmuan S1",
        p1: "Program Studi S1 Teknik Informatika Universitas Muhammadiyah Cirebon berfokus pada penguasaan dasar ilmu komputer, analisis algoritma, serta perancangan perangkat lunak.",
        p2: "Mahasiswa dibekali teori dan praktikum dalam bidang pemrograman, basis data, jaringan, dan kecerdasan buatan untuk menyelesaikan berbagai permasalahan komputasi.",
      };
    }
    if (prodi === "D3") {
      return {
        tag: "Program Diploma (D3) Teknik Informatika",
        headline: "Fokus Pendidikan Vokasi D3",
        p1: "Program Studi D3 Teknik Informatika Universitas Muhammadiyah Cirebon menitikberatkan pada pendidikan keahlian praktis di bidang teknologi informasi.",
        p2: "Kurikulum dirancang dengan porsi praktikum laboratorium yang dominan guna membekali mahasiswa keterampilan teknis pengembangan web, mobile, serta manajemen jaringan.",
      };
    }
    return {
      tag: "Teknik Informatika Universitas Muhammadiyah Cirebon",
      headline: "Profil Program Studi",
      p1: "Program Studi Teknik Informatika Universitas Muhammadiyah Cirebon menyelenggarakan pendidikan tinggi dengan memadukan keahlian teknis dan nilai-nilai moral.",
      p2: "Didukung oleh fasilitas laboratorium serta staf pengajar akademik dan praktisi, kami mempersiapkan mahasiswa untuk berkarier di bidang teknologi maupun melanjutkan studi.",
    };
  };

  const copy = getCopywriting();

  return (
    <section className="relative py-20 md:py-28 overflow-hidden bg-background" id="program-info">
      <div className="absolute top-0 left-0 w-full h-full bg-linear-to-br from-primary/5 via-background to-accent/5 -z-10" />

      <div className="container mx-auto px-6">
        {/* SECTION 1: Intro & Features Grid */}
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${showVisionMission ? "mb-24" : ""}`}>
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-sm font-medium border border-primary/20">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-primary"></span>
              </span>
              {copy.tag}
            </div>

            <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground leading-tight">
              {copy.headline.split(' ').slice(0, -1).join(' ')}{" "}
              <span className="text-primary">{copy.headline.split(' ').slice(-1)}</span>
            </h2>

            <div className="space-y-6 text-lg text-muted-foreground leading-relaxed">
              <p>{copy.p1}</p>
              <p>{copy.p2}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {features.map((feature, index) => (
              <div
                key={index}
                className="group p-6 bg-card border border-border hover:border-accent/50 rounded-2xl shadow-xs hover:shadow-xl hover:shadow-accent/5 transition-all duration-300"
              >
                <div className="w-12 h-12 bg-secondary/20 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                  <feature.icon className="w-6 h-6 text-primary group-hover:text-accent transition-colors" />
                </div>
                <h3 className="font-bold text-lg text-card-foreground mb-2">
                  {feature.title}
                </h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  {feature.description}
                </p>
              </div>
            ))}
          </div>
        </div>

        {/* SECTION 2: Visi & Misi (Separated Cards) */}
        {showVisionMission && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="relative overflow-hidden rounded-2xl bg-primary text-primary-foreground p-8 md:p-10 shadow-xl shadow-primary/20">
              <div className="absolute top-0 right-0 p-4 opacity-10">
                <Compass className="w-32 h-32" />
              </div>
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  <Compass className="w-6 h-6 text-accent-foreground" />
                  <h3 className="text-2xl font-bold">Visi Kami</h3>
                </div>
                <p className="text-primary-foreground/90 text-lg leading-relaxed font-light">
                  {activeData.vision}
                </p>
              </div>
            </div>

            <div className="relative overflow-hidden rounded-2xl bg-card border border-border p-8 md:p-10 shadow-lg">
              <div className="absolute top-0 right-0 p-4 opacity-5">
                <Target className="w-32 h-32 text-foreground" />
              </div>
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-6">
                  <Target className="w-6 h-6 text-accent" />
                  <h3 className="text-2xl font-bold text-foreground">
                    Misi Utama
                  </h3>
                </div>
                <ul className="space-y-4">
                  {missionPoints.map((item, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-muted-foreground"
                    >
                      <span className="mt-1.5 flex h-2 w-2 shrink-0 rounded-full bg-accent" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
            </div>
          </div>
        )}
      </div>
    </section>
  );
}