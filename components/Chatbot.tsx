...
const suggestions = [
  "Berapa biaya pendaftaran PMB?",
  "Apa saja syarat mendaftar?",
  "Dimana lokasi kampus UMC?",
  "Apa saja program studi yang ada?",
];
...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "bot",
      content: "Halo! 👋 Saya asisten virtual resmi PMB Universitas Muhammadiyah Cirebon. Ketik pertanyaan Anda atau pilih di bawah ini!",
    }
  ]);
  ...
  return (
    <div
      className={`fixed z-50 bg-white flex flex-col shadow-2xl transition-all duration-300 border border-border/50 overflow-hidden
        ${isFullScreen 
    ...
    >
      ...
      <div className="bg-linear-to-r from-primary to-primary/90 text-white p-4 flex items-center justify-between shrink-0 shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <div>
            <h3 className="font-bold text-[15px] tracking-wide">PMB AI Assistant</h3>
      ...
      {/* Message Timeline */}
      <div className="flex-1 overflow-y-auto p-4 md:p-5 bg-slate-50/50 space-y-5">
        {messages.map((msg) => (
          <div
            key={msg.id}
          ...
          >
            ...
            <div className={`flex flex-col gap-1 ${msg.role === "user" ? "items-end" : "items-start"}`}>
              <div className={`px-4 py-3 rounded-2xl text-[14px] md:text-[15px] leading-relaxed relative
                ${msg.role === "user" 
      ...
      <div className="p-3 sm:p-4 bg-white border-t border-border mt-auto shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-2 bg-slate-50 rounded-2xl p-1.5 pl-4 border border-border focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20 transition-all shadow-inner">
          <input
            ...
            placeholder="Tanyakan hal seputar PMB..."
